...
class FileSys{
    ...
    ******************************************/
    // helper to find the correct index for a file using probing
    int findIndex(const File& file, int tableSize) const;
    // helper to resize and reorganize the hash table
    void rehash();
    // helper to check if rehashing is needed and start incremental rehashing
    void rehashCheck();
