...
private:
    ...
    Node* merge(Node* p1, Node* p2); // helper to merge two heaps together 
    void clearTree(Node* root);      // helper to clear all nodes in the heap
    Node* copyTree(Node* root);      // helper to copy all nodes for deep copy
    void rebuildHeap();              // helper to rebuild heap after structure or priority change
    void printPreorder(Node* node) const;  // helper to print nodes in preorder
